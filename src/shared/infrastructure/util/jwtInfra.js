...
exports.resolveToken = (token) => {
  try {
    ...
    logger.error(`${MODULE_NAME} resolveToken (ERROR) --> error: ${err.message}`);
    return false;
  }
};

exports.generateToken = (userInfo) => {
  logger.debug(`${MODULE_NAME} generateToken (IN) --> userInfo: ${JSON.stringify(userInfo)}, 
  ...
  logger.debug(`${MODULE_NAME} generateToken (OUT) --> tokenResult: <<tokenResult>>`);
  return tokenResult;
};

// Refreshes the token by an amount of time equals as token_expiration
exports.refreshToken = (token) => {
  logger.debug(`${MODULE_NAME} refreshToken (IN) --> token: <<obfuscated>>`);
  ...
  logger.debug(`${MODULE_NAME} refreshToken (OUT) --> Token refreshed OK`);
  return tokenResult;
};

exports.getUserInfoFromToken = (token) => {
  try {
    logger.debug(`${MODULE_NAME} getUserInfoFromToken (IN) --> token: ${token}`);

    const payload = jwt.decode(token, tokenSecret);
    const userInfo = payload.sub;
    logger.debug(`${MODULE_NAME} getUserInfoFromToken (OUT) --> userInfo: ${JSON.stringify(userInfo)}`);
    return userInfo;
  } catch (error) {
    logger.debug(`${MODULE_NAME} getUserInfoFromToken (ERROR) --> error: ${error.stack}`);
    return null;
  }
};
...
exports.cleanToken = (tokenwithbearer) => {
  try {
    ...
    logger.error(`${MODULE_NAME} (MID) --> tokenwithbearer format not valid`);
    return null;
  }
};
